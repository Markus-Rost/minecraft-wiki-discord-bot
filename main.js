		...
		msg.react('⏳').then( function( reaction ) {
			...
			request( {
				...
				if ( error || !response || !body || !body.query ) {
					...
					else msg.channel.send( 'https://' + wiki + '.gamepedia.com/' + title.toTitle() ).then( message => message.react('440871715938238494') );
				}
				else {
					if ( body.query.pages ) {
...
client.on('message', msg => {
	...
	if ( cont.toLowerCase().includes( process.env.prefix ) && !msg.webhookID && author.id != client.user.id && ( channel.type != 'text' || channel.permissionsFor(client.user).has(['SEND_MESSAGES','ADD_REACTIONS','USE_EXTERNAL_EMOJIS']) ) ) {
		if ( admin(msg) ) channel.send('⚠ **This bot will be removed soon!** ⚠\nPlease invite the new bot and set your wiki and language as it is right now.\n<https://discordapp.com/oauth2/authorize?client_id=461189216198590464&permissions=268954689&scope=bot>\nVisit the Discord server for more information or help: https://discord.gg/v77RTk5');
